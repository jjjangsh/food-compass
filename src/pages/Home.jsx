...
const Home = () => {
  ...
  return (
    <>
      <div className="fixed bottom-3 right-3 flex flex-col gap-5 p-10 mr-4">
        <div className="flex">
          <img
      ...
      <div className="flex flex-col w-full gap-4 justify-center items-center">
        <div className="flex flex-col justify-center items-center gap-4 p-5 rounded-xl my-20">
          ...
          {postsData.length > 0 ? (
            postsData.map((post) => {
              return (
                <div
                  key={post.id}
                  className="flex flex-col w-5/6 border border-gray-300 hover:bg-sky-100 shadow-md p-3 gap-2 justify-center items-center rounded-xl hover:cursor-pointer"
                  onClick={() => navigate(`/postdetail?id=${post.id}`)}
                >
                  {post.image ? (
                    <img src={post.image} alt={post.title} className="h-[173.788px] rounded-xl" />
                  ) : (
                    <div>이미지가 없음</div>
                  )}
                  <p className="font-bold my-1">{post.title}</p>
                  <p className="text-sm whitespace-nowrap">주소 : {post.address}</p>
                </div>
              );
            })
          ) : (
